...
/**
 * Smart ME Chest block: standalone one-cell chest with configurable output face.
 * Shares textures/behavior of AE ME Chest; auto-pushes items like a conveyor on the output side.
 */
public class BlockSmartMEChest extends BlockContainer {

    public BlockSmartMEChest() {
        super(Material.iron);
        setBlockName(PortableMEChest.MODID + ".smart_me_chest");
        setHardness(2.0F);
        setResistance(5.0F);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (!(te instanceof TileSmartMEChest)) {
            return false;
        }
        TileSmartMEChest chest = (TileSmartMEChest) te;
        ItemStack held = player.getHeldItem();

        // Wrench sets output face
        ...
        // Sneak-right-click: extract stored cell
    ...
    public void breakBlock(World world, int x, int y, int z, net.minecraft.block.Block block, int meta) {
        TileEntity te = world.getTileEntity(x, y, z);
        if (te instanceof TileSmartMEChest) {
            ((TileSmartMEChest) te).dropContents();
        }
